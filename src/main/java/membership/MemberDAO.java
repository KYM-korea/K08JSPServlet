...
public class MemberDAO extends JDBConnect{
	//생성자 메소드
	//매개변수가 4개인 부모의 생성자를 호출하여 DB에 연결
	public MemberDAO(String drv, String url, String id, String pw) {
		super(drv,url,id,pw);
	}
	//application 내장객체만 매개변수로 전달한 후 DB에 연결
	public MemberDAO(ServletContext application) {
		super(application);
	}
	/*
	 사용자가 입력한 아이디, 패스워드를 통해 회원테이블 select한 후
	 존재하는 정보인 경우 DTO객체에 해당 정보를 담아 반환
	 */
	public MemberDTO getMemberDTO(String uid, String upass) {
		...
		try {
			...
			//select 쿼리문을 실행한 후 ResultSet으로 반환

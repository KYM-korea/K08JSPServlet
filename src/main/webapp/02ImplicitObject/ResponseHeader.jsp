...
// 응답 헤더에 추가할 값 준비
/* 
get방식으로 전송된 폼값을 날짜 형식을 통해 타임 스탬프로 변경
getTime() : 날짜를 1970년부터 지금까지의 흘러간 시간을 초단위로
	변환해서 반환
	
응답헤더에 날짜를지정하는 경우 대한민국은 세계표준시 +09 즉 9시간이
느리므로 9시간을 더해줘야 정상적인 날짜가 출력
만약 9시 이전의 시간으로 설정되며 어제 날짜가 출력
*/
...
// 문자형은 그대로 사용
// 서버로 전송된 파라미터는 모두 String타입으로만 받기 가능
...
<body>
    <h2>응답 헤더 정보 출력하기</h2>
    <%
    //getHeaderNames()를 통해 응답헤더명 전체를 획득
    Collection<String> headerNames = response.getHeaderNames();
    // 확장 for문으로 반복
    for (String hName : headerNames) {
    	// 헤더명을 통해 헤더값을 획득 후 출력
    ...
    동일한 값 8282가 출력
    getHeader() 메소드의 특성으로 처음 입력한 헤더값만 출력
    */
    %>
    ...
    값이 정상적으로 출력
    즉, add계열의 메소드는 헤더명을 동일하게 사용하더라도
    헤더값은 정상적으로 추가
    */

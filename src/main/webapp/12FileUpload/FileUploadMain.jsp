...
<body>
    ...
    파일 업로드를 위한 FORM태그 작성시 필수사항
    	...
    	2. enctype을 multipart/form-data로 지정
    	
    일반적인 폼값 전송을 위해 enctype을 명시하지 않는 경우
    	request내장객체의 getParameter()로 폼값을 받기 가능
    	
    파일업로드를 위해 enctype을 위와같이 명시하는 경우
    	request내장객체로는 폼값 받기 불가
    	cos.jar확장라이브러리에서 제공하는 MultipartRequest객체를 통해 수령
     -->
     
	<!-- 파일명을 날짜와 시간을 통해 변경하는 방식으로 실무에서 주로 사용하는 방식 -->

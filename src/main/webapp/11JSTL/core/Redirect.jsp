...
redirect태그
: 태그가 실행되는 즉시 페이지가 이동
새로운 페이지에 대한 요청이므로 request영역은 공유되지 않으며,
절대경로 사용시 컨텍스트루트 경로는 제외
 -->
 	<!-- request영역에 변수를 저장 -->
	...
	<!-- response 내장객체를 통한 이동시에는 컨텍스트루트 경로를 반드시 기술
	파라미터가 한글인 경우 깨짐 현상이 발생하므로 URLEncode 클래스를 통해 인코딩 처리가 필수 -->
